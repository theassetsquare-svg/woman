...
// ===== STEP 1: Extract blocks independently =====
...
// ===== STEP 2: Full name insertion with placeholder protection =====
const PH_FULL = '\x00FULL\x00';
const PH_ALT = '\x00ALT\x00';
// Protection placeholder for full names during synonym replacement
...
for (const block of blocks) {
  ...
  // Clean remaining "여기"/"이 가게" anywhere else → "이곳"
  ...
  // ===== PROTECT all full name occurrences before synonym replacement =====
...
// ===== STEP 4: Synonym-based dup reduction (with full names protected) =====
const synonymSets = {
  '프리미엄': ['하이엔드', '상위급', '고급', '특급'],
...
for (let i = newMatches.length - 1; i >= 0; i--) {
  ...
  for (const [word, syns] of Object.entries(synonymSets)) {
    // Count occurrences OUTSIDE protected zones
    const regex = new RegExp(word, 'g');
    const allMatches = [...block.matchAll(regex)];
    // Only count those NOT inside protection markers
    const unprotected = allMatches.filter(m => {
    ...
    if (unprotected.length > 5) {
      ...
      block = block.replace(regex, (m, offset) => {
        // Check if inside protection zone
        const before = block.substring(Math.max(0, offset - 20), offset);
        if (before.includes(PH_PROTECT)) return m; // Don't touch protected
        c++;
...
// ===== STEP 6: Count and fix LOW venues =====
// Recount after all transformations
...
// For LOW venues, directly inject full name into body/conclusion by replacing "이곳" instances
function boostVenue(venueId, targetTotal) {
  ...
  // Boost body sections first
  if (counts.sc < 2) {
    const bodyNeeded = 2 - counts.sc;
    const bodyRe = /(body:\s*`)([^`]*?)(`)/g;
    let replaced = 0;
    block = block.replace(bodyRe, (match, p1, bodyText, p3) => {
      if (replaced >= bodyNeeded) return match;
      // Replace first "이곳은"/"이곳의"/"이곳을"/"이곳에서" with fullName variant
      let done = false;
      bodyText = bodyText.replace(/이곳은/, (m) => {
        if (!done && replaced < bodyNeeded) { done = true; replaced++; return info.full + '은'; }
        return m;
      });
      if (!done) {
        bodyText = bodyText.replace(/이곳의/, (m) => {
          if (!done && replaced < bodyNeeded) { done = true; replaced++; return info.full + '의'; }
          return m;
        });
      }
      if (!done) {
        bodyText = bodyText.replace(/이곳을/, (m) => {
          if (!done && replaced < bodyNeeded) { done = true; replaced++; return info.full + '을'; }
          return m;
        });
      }
      if (!done) {
        bodyText = bodyText.replace(/이곳에서/, (m) => {
          if (!done && replaced < bodyNeeded) { done = true; replaced++; return info.full + '에서'; }
          return m;
        });
      }
      if (!done) {
        bodyText = bodyText.replace(/이곳/, (m) => {
          if (!done && replaced < bodyNeeded) { done = true; replaced++; return info.full; }
          return m;
        });
      }
  ...
  if (needed > 0) {
    ...
    if (concM) {
      ...
      // Replace "이곳" variants with fullName
      const replacements = [
        [/이곳은/, info.full + '은'],
        [/이곳의/, info.full + '의'],
        [/이곳을/, info.full + '을'],
      ...
      for (const [regex, replacement] of replacements) {
        ...
        concText = concText.replace(regex, () => {
          if (concReplaced < concNeeded) { concReplaced++; return replacement; }
          return regex.source;
        });
      }
  ...
  if (needed > 0) {
    ...
    if (introM) {
      ...
      for (const [regex, replacement] of replacements) {
        ...
        introText = introText.replace(regex, () => {
          if (introReplaced < needed) { introReplaced++; return replacement; }
          return regex.source;
        });
      }
...
// Check and boost all venues
for (const venueId of Object.keys(venueMap)) {
  const counts = countFull(venueId);
  if (counts && counts.total < 6) {
    console.log(`Boosting ${venueId}: ${counts.ic}/${counts.sc}/${counts.cc} = ${counts.total} → target 7`);
    boostVenue(venueId, 7);
  }
}
...
for (let i = 0; i < fMatches.length; i++) {
  ...
  if (t < 6) lowCount++;
  if (t > 8) highCount++;
  console.log(`${venueId.padEnd(27)} ${info.full.padEnd(25)} ${String(ic).padStart(4)} ${String(sc).padStart(4)} ${String(cc).padStart(4)} ${String(t).padStart(4)}  ${st}`);
}
...
for (let i = 0; i < fMatches.length; i++) {
  ...
  for (const [otherId, otherInfo] of Object.entries(venueMap)) {
    ...
    // Skip if other's full name is a substring of this venue's full name
