...
const catConfig: Record<CatKey, { label: string; plural: string; title: string; desc: string; intro: string }> = {
  ...
  night: {
    ...
    intro: `나이트는 세대를 넘어 사람들이 모이는 곳입니다. 20대부터 60대까지, 라이브 밴드 앞에서 춤추는 그 순간만큼은 나이가 의미 없습니다.\n\n전국 나이트를 하나하나 직접 확인했습니다. 분위기는 어떤지, 어떤 음악이 나오는지, 주차는 되는지, 언제 가야 좋은지.\n\n실장이 직접 관리하는 곳, 단골이 많은 곳, 주말이면 줄 서는 곳. 가기 전에 여기서 확인하세요. 전화 한 통이면 현장에서 당황할 일이 없습니다.`,
  },
  lounge: {
    label: '라운지',
    plural: 'lounges',
    ...
    intro: `라운지는 시끄러운 곳이 싫은 사람들을 위한 곳입니다. 조용한 음악, 편안한 소파, 좋은 술 한 잔. 대화가 가능한 밤을 원한다면 라운지가 정답입니다.\n\n소파석에 앉아 DJ 셋을 감상하거나, 바 카운터에서 칵테일을 즐기거나. 에너지 넘치는 파티보다 여유로운 분위기를 선호하는 분께 추천합니다.\n\n압구정 신사동 일대의 분위기 좋은 라운지만 골라 정리했습니다. 예약이 필수인 곳이 많으니 사전 문의를 추천합니다.`,
  },
  room: {
    label: '룸',
    plural: 'rooms',
    title: '프라이빗 룸 — 우리끼리 즐기는 특별한 공간',
    desc: '회식, 모임, 특별한 날. 프라이빗 룸 정보를 비교하세요.',
    intro: `룸은 프라이빗한 공간이 필요한 사람들의 선택입니다. 회식이든, 친구 모임이든, 특별한 날이든. 다른 사람 신경 쓰지 않고 우리끼리 즐기고 싶을 때 가는 곳입니다.\n\n넓은 공간에 편한 소파, 전용 음향 시스템. 인원에 맞는 룸을 고르면 분위기가 확 달라집니다.\n\n어디가 넓은지, 어디가 서비스 좋은지, 예약은 어떻게 하는지. 방문 전 전화 한 통이면 완벽한 밤이 됩니다.`,
  },
  yojeong: {
    label: '요정',
    plural: 'yojeong',
    title: '한국 전통 요정 — 한정식과 국악이 어우러지는 공간',
    desc: '접대, 비즈니스, 특별한 모임. 전통 요정의 격조 있는 경험.',
    intro: `요정은 한국 전통 문화를 경험하는 특별한 공간입니다. 15가지 이상의 한정식에 국악 라이브, 단아한 한복 서비스. 격조 있는 분위기에서 접대하거나 비즈니스 모임을 진행하기에 요정만 한 곳이 없습니다.\n\n수십 개의 프라이빗 룸이 있어 인원과 목적에 맞게 선택할 수 있습니다. 정찰제로 운영되어 추가 비용 걱정이 없다는 것도 장점입니다.\n\n특별한 날, 격이 다른 자리가 필요할 때. 여기서 미리 확인하세요.`,
  },
  hoppa: {
    label: '호빠',
    plural: 'hoppa',
    title: '호빠 — 여성이 편하게 즐기는 프리미엄 공간',
...
const firstVisitGuide: Record<CatKey, { q: string; a: string }[]> = {
  ...
  yojeong: [
    { q: '요정은 어떤 곳인가요?', a: '한정식 코스와 함께 국악 공연을 즐기는 전통 접대 공간입니다. 비즈니스 모임이나 특별한 자리에 적합합니다.' },
  ...
  hoppa: [
    ...
    { q: '어떤 옷을 입고 가나요?', a: '편하게 입고 가시면 됩니다. 특별한 드레스코드는 없습니다.' },
  ],
};
