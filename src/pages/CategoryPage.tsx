...
const catConfig: Record<CatKey, { label: string; plural: string; title: string; desc: string; intro: string }> = {
  club: {
    ...
    desc: '강남·홍대·이태원·부산까지 현장 검증한 클럽만 모았습니다. 줄 길이·사운드·드레스코드까지 가기 전에 확인하고 오늘 밤 실패 없는 한 곳을 고르세요.',
  ...
  night: {
    ...
    desc: '서울·부산·대구·광주 전국 나이트를 한자리에서 비교하세요. 부킹 분위기·음악·주차·가야 할 시간까지 발로 뛰어 정리했습니다. 전화 한 통이면 헤맬 일 없습니다.',
  ...
  lounge: {
    ...
    desc: '시끄러운 클럽이 부담된다면 라운지. 편안한 소파·좋은 술·조용한 음악까지, 대화가 되는 압구정 라운지만 골라 분위기와 예약 정보를 정리했습니다.',
  ...
  room: {
    ...
    desc: '회식·모임·특별한 밤을 우리끼리. 넓은 공간과 전용 음향을 갖춘 프라이빗 룸을 인원·서비스·예약 조건까지 비교해 후회 없는 자리를 고르세요.',
    intro: `프라이빗한 공간이 필요한 사람들의 선택입니다. 회식이든, 친구 모임이든, 기억할 밤이든. 다른 사람 신경 쓰지 않고 우리끼리 즐기고 싶을 때 가는 곳입니다.\n\n넓은 공간에 편한 소파, 전용 음향 시스템. 인원에 맞는 개인 공간을 고르면 분위기가 확 달라집니다.\n\n어디가 넓은지, 어디가 서비스 좋은지, 예약은 어떻게 하는지. 방문 전 전화 한 통이면 제대로 된 밤이 됩니다.`,
  },
  yojeong: {
    label: '요정',
    plural: 'yojeong',
    title: '한국 전통 요정 — 한정식과 국악이 어우러지는 공간',
    desc: '접대·비즈니스·격 있는 자리라면 전통 요정. 한정식과 국악이 어우러지는 격조 있는 경험을 정찰제로 안심하고 즐기세요. 사전 예약 정보까지 정리했습니다.',
    intro: `한국 전통 문화를 경험하는 남다른 공간입니다. 15가지 이상의 한정식에 국악 라이브, 단아한 한복 서비스. 격조 있는 분위기에서 접대하거나 비즈니스 모임을 진행하기에 이만한 곳이 없습니다.\n\n수십 개의 프라이빗 개인실이 있어 인원과 목적에 맞게 선택된다. 정찰제로 운영되어 추가 비용 걱정이 없다는 것도 장점입니다.\n\n기억할 밤, 격이 다른 자리가 필요할 때. 여기서 미리 확인하세요.`,
  },
  hoppa: {
    label: '호빠',
    plural: 'hoppa',
    title: '호빠 — 여성이 편하게 즐기는 고급 공간',
    desc: '친절한 호스트와 부담 없는 시간. 처음이라도 걱정 마세요 — 분위기·시스템·첫 방문 팁까지 미리 확인하고 가면 누구나 편하게 즐길 수 있습니다.',
...
const firstVisitGuide: Record<CatKey, { q: string; a: string }[]> = {
  ...
  yojeong: [
    ...
    { q: '정찰제가 뭔가요?', a: '처음 안내받은 금액이 나갈 때도 동일합니다. 추가 요금이 없어 안심하고 이용된다.' },
